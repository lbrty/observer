...
func TestDocumentHandler_List_Success(t *testing.T) {
	...
	docRepo.EXPECT().List(gomock.Any(), personID).Return([]*document.Document{
		{ID: handlertest.TestID().String(), PersonID: personID, ProjectID: handlertest.TestID().String(), Name: "passport.pdf", Path: "a/b/c", MimeType: "application/pdf", Size: 1024, CreatedAt: now},
	}, nil)
...
func TestDocumentHandler_Get_NoPermission(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodGet, "/projects/x/documents/"+id, nil, gin.Params{
		{Key: "id", Value: id},
	})
	setCanViewDocuments(c, false)
	h.Get(c)
...
func TestDocumentHandler_Get_Success(t *testing.T) {
	...
	docRepo.EXPECT().GetByID(gomock.Any(), id).Return(&document.Document{
		ID: id, PersonID: handlertest.TestID().String(), ProjectID: projectID,
		Name: "passport.pdf", Path: "a/b/c", MimeType: "application/pdf", Size: 1024, CreatedAt: now,
	}, nil)

	c, w := handlertest.NewTestContextWithParams(http.MethodGet, "/projects/"+projectID+"/documents/"+id, nil, gin.Params{
		{Key: "project_id", Value: projectID},
		{Key: "id", Value: id},
	})
	setCanViewDocuments(c, true)
...
func TestDocumentHandler_Update_NotFound(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodPatch, "/projects/x/documents/"+id, map[string]any{
		"name": "updated.pdf",
	}, gin.Params{
		{Key: "id", Value: id},
	})
...
func TestDocumentHandler_Update_Success(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodPatch, "/projects/"+projectID+"/documents/"+id, map[string]any{
		"name": "renamed.pdf",
	}, gin.Params{
		{Key: "project_id", Value: projectID},
		{Key: "id", Value: id},
...
func TestDocumentHandler_Delete_NotFound(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodDelete, "/projects/x/documents/"+id, nil, gin.Params{
		{Key: "id", Value: id},
	})
	setCanViewDocuments(c, true)
	h.Delete(c)
...
func TestDocumentHandler_Delete_Success(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodDelete, "/projects/"+projectID+"/documents/"+id, nil, gin.Params{
		{Key: "project_id", Value: projectID},
		{Key: "id", Value: id},
	})
	setCanViewDocuments(c, true)
