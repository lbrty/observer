...
func newStateHandler(ctrl *gomock.Controller) (*admin.StateHandler, *repomock.MockStateRepository) {
	repo := repomock.NewMockStateRepository(ctrl)
	uc := ucadmin.NewStateUseCase(repo)
	return admin.NewStateHandler(uc), repo
}
...
func TestStateHandler_List_AllStates(t *testing.T) {
	...
	repo.EXPECT().ListAll(gomock.Any()).Return([]*reference.State{
		{ID: handlertest.TestID().String(), CountryID: handlertest.TestID().String(), Name: "Bishkek", CreatedAt: now, UpdatedAt: now},
	}, nil)

	c, w := handlertest.NewTestContext(http.MethodGet, "/admin/states", nil)
	h.List(c)
...
func TestStateHandler_List_ByCountry(t *testing.T) {
	...
	now := time.Now().UTC()
	countryID := handlertest.TestID().String()
	repo.EXPECT().List(gomock.Any(), countryID).Return([]*reference.State{
		{ID: handlertest.TestID().String(), CountryID: countryID, Name: "Osh", CreatedAt: now, UpdatedAt: now},
	}, nil)

	c, w := handlertest.NewTestContext(http.MethodGet, "/admin/states?country_id="+countryID, nil)
	h.List(c)
...
func TestStateHandler_Get_NotFound(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodGet, "/admin/states/"+id, nil, gin.Params{
		{Key: "id", Value: id},
	})
	h.Get(c)
...
func TestStateHandler_Create_NoCountryID(t *testing.T) {
	...
	c, w := handlertest.NewTestContext(http.MethodPost, "/admin/states", map[string]any{
		"name": "Bishkek",
	})
	h.Create(c)
...
func TestStateHandler_Create_ValidationError(t *testing.T) {
	...
	countryID := handlertest.TestID().String()
	c, w := handlertest.NewTestContext(http.MethodPost, "/admin/states?country_id="+countryID, map[string]any{})
	h.Create(c)
...
func TestStateHandler_Update_NotFound(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodPatch, "/admin/states/"+id, map[string]any{
		"name": "Updated",
...
func TestStateHandler_Update_Success(t *testing.T) {
	...
	existing := &reference.State{
		ID: id, CountryID: handlertest.TestID().String(), Name: "Bishkek", CreatedAt: now, UpdatedAt: now,
	}
...
func TestStateHandler_Delete_NotFound(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodDelete, "/admin/states/"+id, nil, gin.Params{
		{Key: "id", Value: id},
	})
	h.Delete(c)
