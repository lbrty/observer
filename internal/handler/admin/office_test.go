...
func newOfficeHandler(ctrl *gomock.Controller) (*admin.OfficeHandler, *repomock.MockOfficeRepository) {
	repo := repomock.NewMockOfficeRepository(ctrl)
	uc := ucadmin.NewOfficeUseCase(repo)
	return admin.NewOfficeHandler(uc), repo
}
...
func TestOfficeHandler_List_Success(t *testing.T) {
	...
	now := time.Now().UTC()
	placeID := handlertest.TestID().String()
	repo.EXPECT().List(gomock.Any()).Return([]*reference.Office{
		{ID: handlertest.TestID().String(), Name: "Main Office", PlaceID: &placeID, CreatedAt: now, UpdatedAt: now},
		{ID: handlertest.TestID().String(), Name: "Branch Office", CreatedAt: now, UpdatedAt: now},
	}, nil)

	c, w := handlertest.NewTestContext(http.MethodGet, "/admin/offices", nil)
	h.List(c)
...
func TestOfficeHandler_Get_NotFound(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodGet, "/admin/offices/"+id, nil, gin.Params{
		{Key: "id", Value: id},
	})
	h.Get(c)
...
func TestOfficeHandler_Create_ValidationError(t *testing.T) {
	...
	c, w := handlertest.NewTestContext(http.MethodPost, "/admin/offices", map[string]any{})
	h.Create(c)
...
func TestOfficeHandler_Update_NotFound(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodPatch, "/admin/offices/"+id, map[string]any{
		"name": "Updated",
...
func TestOfficeHandler_Delete_NotFound(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodDelete, "/admin/offices/"+id, nil, gin.Params{
		{Key: "id", Value: id},
	})
	h.Delete(c)
