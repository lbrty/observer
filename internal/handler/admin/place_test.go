...
func newPlaceHandler(ctrl *gomock.Controller) (*admin.PlaceHandler, *repomock.MockPlaceRepository) {
	repo := repomock.NewMockPlaceRepository(ctrl)
	uc := ucadmin.NewPlaceUseCase(repo)
	return admin.NewPlaceHandler(uc), repo
}
...
func TestPlaceHandler_List_AllPlaces(t *testing.T) {
	...
	repo.EXPECT().ListAll(gomock.Any()).Return([]*reference.Place{
		{ID: handlertest.TestID().String(), StateID: handlertest.TestID().String(), Name: "Bishkek City", CreatedAt: now, UpdatedAt: now},
	}, nil)

	c, w := handlertest.NewTestContext(http.MethodGet, "/admin/places", nil)
	h.List(c)
...
func TestPlaceHandler_List_ByState(t *testing.T) {
	...
	now := time.Now().UTC()
	stateID := handlertest.TestID().String()
	repo.EXPECT().List(gomock.Any(), stateID).Return([]*reference.Place{
		{ID: handlertest.TestID().String(), StateID: stateID, Name: "Osh City", CreatedAt: now, UpdatedAt: now},
	}, nil)

	c, w := handlertest.NewTestContext(http.MethodGet, "/admin/places?state_id="+stateID, nil)
	h.List(c)
...
func TestPlaceHandler_Get_NotFound(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodGet, "/admin/places/"+id, nil, gin.Params{
		{Key: "id", Value: id},
	})
	h.Get(c)
...
func TestPlaceHandler_Create_NoStateID(t *testing.T) {
	...
	c, w := handlertest.NewTestContext(http.MethodPost, "/admin/places", map[string]any{
		"name": "Bishkek",
	})
	h.Create(c)
...
func TestPlaceHandler_Create_ValidationError(t *testing.T) {
	...
	stateID := handlertest.TestID().String()
	c, w := handlertest.NewTestContext(http.MethodPost, "/admin/places?state_id="+stateID, map[string]any{})
	h.Create(c)
...
func TestPlaceHandler_Update_NotFound(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodPatch, "/admin/places/"+id, map[string]any{
		"name": "Updated",
...
func TestPlaceHandler_Update_Success(t *testing.T) {
	...
	existing := &reference.Place{
		ID: id, StateID: handlertest.TestID().String(), Name: "Bishkek", CreatedAt: now, UpdatedAt: now,
	}
...
func TestPlaceHandler_Delete_NotFound(t *testing.T) {
	...
	c, w := handlertest.NewTestContextWithParams(http.MethodDelete, "/admin/places/"+id, nil, gin.Params{
		{Key: "id", Value: id},
	})
	h.Delete(c)
