...
func (h *PlaceHandler) List(c *gin.Context) {
	...
	if err != nil {
		handler.InternalError(c, "list places", err)
		return
	}
...
func (h *PlaceHandler) Create(c *gin.Context) {
	stateID := c.Query("state_id")
	if stateID == "" {
		c.JSON(http.StatusBadRequest, handler.ErrJSON("errors.validation", "state_id is required"))
		return
	}
	input, ok := handler.BindJSON[ucadmin.CreatePlaceInput](c)
	if !ok {
		return
	}
...
func (h *PlaceHandler) Update(c *gin.Context) {
	input, ok := handler.BindJSON[ucadmin.UpdatePlaceInput](c)
	if !ok {
		return
	}
