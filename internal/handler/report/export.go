...
func (h *ExportHandler) ExportPeople(c *gin.Context) {
	...
	if err != nil {
		handler.InternalError(c, "export people", err)
		return
	}
...
func (h *ExportHandler) ExportSupportRecords(c *gin.Context) {
	...
	if err != nil {
		handler.InternalError(c, "export support records", err)
		return
	}
...
func (h *ExportHandler) ExportPets(c *gin.Context) {
	...
	if err != nil {
		handler.InternalError(c, "export pets", err)
		return
	}
...
func (h *ExportHandler) ExportHouseholds(c *gin.Context) {
	...
	if err != nil {
		handler.InternalError(c, "export households", err)
		return
	}
	...
	for _, hh := range out.Households {
		_ = w.Write([]string{
			hh.ID,
			ptrStr(hh.ReferenceNumber),
			ptrStr(hh.HeadPersonID),
			strconv.Itoa(hh.MemberCount),
			hh.CreatedAt.Format("2006-01-02T15:04:05Z"),
		})
	}
	w.Flush()
