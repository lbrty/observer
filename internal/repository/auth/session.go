...
// NewSession creates a SessionRepository backed by the given DB.
func NewSession(db *sqlx.DB) repository.SessionRepository {
...
func (r *sessionRepo) scanSession(row *sql.Row) (*auth.Session, error) {
	...
	s.ID = id
	s.UserID = userID
	repository.TimesToUTC(&expiresAt, &createdAt)
