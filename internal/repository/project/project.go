...
// New creates a ProjectRepository.
func New(db *sqlx.DB) repository.ProjectRepository {
...
func (r *projectRepo) List(ctx context.Context, filter project.ProjectListFilter) ([]*project.Project, int, error) {
	...
	countSQL, countArgs, err := repository.PSQL.Select("COUNT(*)").From("projects").Where(cond).ToSql()
	if err != nil {
	...
	listSQL, listArgs, err := repository.PSQL.Select("id, name, description, owner_id, status, created_at, updated_at").
...
func (r *projectRepo) Update(ctx context.Context, p *project.Project) error {
	...
	return repository.CheckRowsAffected(res, project.ErrProjectNotFound)
}
