...
// NewState creates a StateRepository.
func NewState(db *sqlx.DB) repository.StateRepository {
...
func scanState(row interface{ Scan(dest ...any) error }) (*reference.State, error) {
	...
	repository.TimesToUTC(&s.CreatedAt, &s.UpdatedAt)
	return &s, nil
}
...
func (r *stateRepo) Update(ctx context.Context, s *reference.State) error {
	...
	return repository.CheckRowsAffected(res, reference.ErrStateNotFound)
}
