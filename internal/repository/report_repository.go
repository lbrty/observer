...
func peopleFilterCond(f report.ReportFilter, dateCol string) sq.And {
	cond := sq.And{}
	if f.DateFrom != nil {
...
func supportFilterCond(f report.ReportFilter, dateCol string) sq.And {
	cond := sq.And{}
	if f.DateFrom != nil {
		cond = append(cond, sq.GtOrEq{dateCol: *f.DateFrom})
	}
	...
	if len(personCond) > 0 {
		subSQL, subArgs, _ := psql.Select("id").From("people").Where(personCond).ToSql()
