...
// NewCredentials creates a CredentialsRepository backed by the given DB.
func NewCredentials(db *sqlx.DB) repository.CredentialsRepository {
	return &credentialsRepo{db: db}
}

func (r *credentialsRepo) Create(ctx context.Context, cred *domainuser.Credentials) error {
	const q = `
...
func (r *credentialsRepo) Update(ctx context.Context, cred *domainuser.Credentials) error {
	const q = `
		UPDATE credentials
...
func (r *credentialsRepo) GetByUserID(ctx context.Context, userID ulid.ULID) (*domainuser.Credentials, error) {
	const q = `SELECT user_id, password_hash, salt, updated_at FROM credentials WHERE user_id=$1`

	var cred domainuser.Credentials
	...
	cred.UserID = id
	repository.TimesToUTC(&updatedAt)
