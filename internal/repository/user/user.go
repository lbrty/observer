...
// New creates a UserRepository backed by the given DB.
...
func (r *userRepo) Update(ctx context.Context, u *user.User) error {
	...
	return repository.CheckRowsAffected(res, user.ErrUserNotFound)
}
...
func (r *userRepo) List(ctx context.Context, filter user.UserListFilter) ([]*user.User, int, error) {
	...
	countSQL, countArgs, err := repository.PSQL.Select("COUNT(*)").From("users").Where(cond).ToSql()
	if err != nil {
	...
	listSQL, listArgs, err := repository.PSQL.Select(userColumns).From("users").Where(cond).
...
func scanUserRow(row interface{ Scan(dest ...any) error }) (*user.User, error) {
	...
	repository.TimesToUTC(&createdAt, &updatedAt)
