...
// NewMFA creates an MFARepository backed by the given DB.
func NewMFA(db *sqlx.DB) repository.MFARepository {
	return &mfaRepo{db: db}
}

func (r *mfaRepo) Create(ctx context.Context, cfg *domainuser.MFAConfig) error {
	const q = `
...
func (r *mfaRepo) GetByUserID(ctx context.Context, userID ulid.ULID) (*domainuser.MFAConfig, error) {
	const q = `SELECT user_id, method, secret, phone, is_enabled, created_at FROM mfa_configs WHERE user_id=$1`

	var cfg domainuser.MFAConfig
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, domainuser.ErrMFAConfigNotFound
		}
	...
	cfg.UserID = id
	repository.TimesToUTC(&createdAt)
	cfg.CreatedAt = createdAt

	return &cfg, nil
}

func (r *mfaRepo) Upsert(ctx context.Context, cfg *domainuser.MFAConfig) error {
	const q = `
		INSERT INTO mfa_configs (user_id, method, secret, phone, is_enabled, created_at)
		VALUES ($1, $2, $3, $4, $5, $6)
