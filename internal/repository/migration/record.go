...
// New creates a MigrationRecordRepository.
func New(db *sqlx.DB) repository.MigrationRecordRepository {
...
func scanMigration(row interface{ Scan(dest ...any) error }) (*migration.Record, error) {
	...
	repository.TimesToUTC(&r.CreatedAt)
	if updatedAt.Valid {
...
func (r *migrationRecordRepo) Update(ctx context.Context, rec *migration.Record) error {
	...
	return repository.CheckRowsAffected(res, migration.ErrRecordNotFound)
}
