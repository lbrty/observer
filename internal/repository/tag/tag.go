...
// New creates a TagRepository.
func New(db *sqlx.DB) repository.TagRepository {
...
func scanTag(row interface{ Scan(dest ...any) error }) (*tag.Tag, error) {
	...
	repository.TimesToUTC(&t.CreatedAt)
	return &t, nil
}
...
func (r *tagRepo) Update(ctx context.Context, t *tag.Tag) error {
	...
	return repository.CheckRowsAffected(res, tag.ErrTagNotFound)
}
