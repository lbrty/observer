...
// New creates a HouseholdRepository.
func New(db *sqlx.DB) repository.HouseholdRepository {
...
func scanHousehold(row interface{ Scan(dest ...any) error }) (*household.Household, error) {
	...
	repository.TimesToUTC(&h.CreatedAt, &h.UpdatedAt)
	return &h, nil
}
...
func (r *householdRepo) List(ctx context.Context, filter household.HouseholdListFilter) ([]*household.Household, int, error) {
	...
	countSQL, countArgs, err := repository.PSQL.Select("COUNT(*)").From("households h").Where(cond).ToSql()
	if err != nil {
...
func (r *householdRepo) Update(ctx context.Context, h *household.Household) error {
	...
	return repository.CheckRowsAffected(res, household.ErrHouseholdNotFound)
}
...
// NewMember creates a HouseholdMemberRepository.
func NewMember(db *sqlx.DB) repository.HouseholdMemberRepository {
...
func (r *householdMemberRepo) Remove(ctx context.Context, householdID, personID string) error {
	...
	return repository.CheckRowsAffected(res, household.ErrMemberNotFound)
}
