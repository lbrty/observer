...
// NewNote creates a PersonNoteRepository.
func NewNote(db *sqlx.DB) repository.PersonNoteRepository {
...
func scanNote(row interface{ Scan(dest ...any) error }) (*note.Note, error) {
	...
	repository.TimesToUTC(&n.CreatedAt)
	if updatedAt.Valid {
...
func (r *personNoteRepo) Update(ctx context.Context, n *note.Note) error {
	...
	return repository.CheckRowsAffected(res, note.ErrNoteNotFound)
}
