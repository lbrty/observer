...
// NewDocument creates a DocumentRepository.
func NewDocument(db *sqlx.DB) repository.DocumentRepository {
...
func scanDocument(row interface{ Scan(dest ...any) error }) (*document.Document, error) {
	...
	repository.TimesToUTC(&d.CreatedAt)
	if updatedAt.Valid {
...
func (r *documentRepo) Create(ctx context.Context, d *document.Document) error {
	const q = `INSERT INTO documents (
		id, person_id, project_id, uploaded_by, encryption_key_ref, name, path, mime_type, size, created_at
	)
...
func (r *documentRepo) Update(ctx context.Context, d *document.Document) error {
	...
	return repository.CheckRowsAffected(res, document.ErrDocumentNotFound)
}
