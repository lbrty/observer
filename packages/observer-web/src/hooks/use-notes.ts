...
export function useNotes(projectId: string, personId: string) {
  return useQuery({
    ...
    queryFn: () =>
      api
        .get(`projects/${projectId}/people/${personId}/notes`)
...
export function useCreateNote(projectId: string, personId: string) {
  ...
  return useMutation({
    mutationFn: (data: CreateNoteInput) =>
      api
        .post(`projects/${projectId}/people/${personId}/notes`, { json: data })
        .json<Note>(),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["notes", projectId, personId] }),
  });
}
...
export function useDeleteNote(projectId: string, personId: string) {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      api.delete(`projects/${projectId}/people/${personId}/notes/${id}`),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["notes", projectId, personId] }),
  });
}
