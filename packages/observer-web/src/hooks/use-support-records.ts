...
export function useSupportRecords(
  projectId: string,
  params: ListSupportRecordsParams = {},
) {
  return useQuery({
...
export function useSupportRecord(projectId: string, id: string) {
  return useQuery({
    ...
    queryFn: () =>
      api
        .get(`projects/${projectId}/support-records/${id}`)
        .json<SupportRecord>(),
...
export function useCreateSupportRecord(projectId: string) {
  ...
  return useMutation({
    mutationFn: (data: CreateSupportRecordInput) =>
      api
        .post(`projects/${projectId}/support-records`, { json: data })
        .json<SupportRecord>(),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["support-records", projectId] }),
  });
}
...
export function useDeleteSupportRecord(projectId: string) {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      api.delete(`projects/${projectId}/support-records/${id}`),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["support-records", projectId] }),
  });
}
