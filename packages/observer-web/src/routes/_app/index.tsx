        ...
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {stats.map((stat) => (
              ...
              >
                <p className="text-[11px] font-semibold uppercase tracking-wide text-fg-tertiary">
                  {stat.label}
                </p>
          ...
          <h2 className="mt-8 mb-4 text-lg font-semibold text-fg">
            ...
            {quickActions.map(
              ({ to, icon: ActionIcon, color, titleKey, descKey }) => (
                <Link
                  key={to}
                  to={to}
                  className="group rounded-xl border border-border-secondary bg-bg-secondary p-5 transition-shadow hover:shadow-elevated"
                >
                  <span
                    className={`mb-4 inline-flex size-10 items-center justify-center rounded-xl ${colorClasses[color]}`}
                  >
