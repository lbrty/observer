...
export function UISwitch({ checked, onCheckedChange, label, disabled }: UISwitchProps) {
  return (
    <label className="inline-flex cursor-pointer items-center gap-2.5 text-sm text-fg-secondary select-none">
      ...
      >
        <Switch.Thumb className="pointer-events-none block size-3.5 translate-x-0.5 rounded-full bg-fg-tertiary shadow-sm transition-transform data-checked:translate-x-4.5 data-checked:bg-accent-fg" />
      </Switch.Root>
      {label}
    </label>
  );
