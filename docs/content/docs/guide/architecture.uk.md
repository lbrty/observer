...
Ця сторінка призначена для розробників і технічного персоналу, які хочуть зрозуміти, як побудований Observer. Якщо ви адміністратор, що налаштовує Observer для своєї організації, можете це пропустити — перейдіть до [Розгортання](/docs/guide/deployment/).

## Загальний огляд

Кожен HTTP-запит проходить однаковий шлях: він надходить на сервер, проходить через middleware (автентифікація, логування), потрапляє до обробника, який делегує виконання use case, а use case взаємодіє з базою даних через repository. Конфігурація та впровадження залежностей з'єднують усе разом при запуску.
...
## Потік залежностей (Clean Architecture)

Кодова база організована шарами. Внутрішні шари визначають правила, зовнішні шари забезпечують інфраструктуру. Залежності завжди спрямовані всередину — бізнес-логіка ніколи не імпортує код бази даних чи HTTP напряму. Це дає змогу тестувати use case без запущеної бази даних.
...
## Repository: від інтерфейсу до реалізації

Доменний код визначає, _які_ операції з даними потрібні (інтерфейси), тоді як шар PostgreSQL забезпечує _як_ (реалізації). Таке розділення означає, що можна замінити PostgreSQL на іншу базу даних, не змінюючи жодної бізнес-логіки. Кожна доменна область — користувачі, автентифікація, проєкти, довідкові дані — має власний інтерфейс repository.
...
Кожна дія користувача — вхід у систему, перегляд списку людей, призначення дозволів — обробляється окремим use case. Use cases координують роботу між repositories та криптографічними сервісами, але самі не містять HTTP- чи SQL-коду. Діаграма нижче показує, від яких repositories залежить кожен use case.
...
Ось що відбувається, коли користувач входить у систему. Запит надходить через маршрутизатор, проходить через middleware, що присвоює ідентифікатор запиту та логер, потім потрапляє до обробника автентифікації. Обробник розбирає JSON-тіло та викликає use case входу, який знаходить користувача, перевіряє пароль за допомогою Argon2, генерує JWT-токени та створює сесію.
...
Захищені маршрути проходять додаткові перевірки. Адміністративні маршрути перевіряють платформну роль користувача (admin, staff тощо). Маршрути в межах проєкту завантажують проєктний дозвіл користувача та перевіряють, чи достатня його проєктна роль для запитуваної дії. Middleware також встановлює прапорці чутливості, що контролюють, чи включає відповідь контактну інформацію, особисті дані або дані документів.
...
При запуску застосунок зчитує конфігурацію та підключається до бази даних, а потім з'єднує все в контейнері впровадження залежностей. Контейнер створює repositories, криптографічні сервіси та use cases, передаючи кожному компоненту його залежності. Повністю зібраний контейнер передається серверу, який вбудовує обробники та middleware в маршрутизатор.
