...
just test-all          # усі тести, включно з інтеграційними
...
| Шаблон            | Призначення                                   |
| ----------------- | --------------------------------------------- |
| `*_test.go`       | Тестовий файл у тому самому пакеті            |
...
Інтеграційні тести запускають справжні Docker-контейнери для Postgres та Redis.
...
| Перевизначення конфігурації через env | Unit | Змінні оточення перевизначають значення |
