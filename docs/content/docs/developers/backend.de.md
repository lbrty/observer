...
`bindJSON[T]` ist in `internal/handler/errors.go` definiert. Sie dekodiert den Request-Body und schreibt bei Fehler eine `400`-Antwort; sie gibt `false` zurück, damit der Handler sofort beenden kann.
