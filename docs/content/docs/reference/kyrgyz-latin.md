---
title: Kyrgyz Latin Transliteration
weight: 2
---

## Vowel Classification

| Type         | Vowels             | Used with |
| ------------ | ------------------ | --------- |
| Back (жоон)  | A a, O o, U u, Y y | Q q, Ğ ğ  |
| Front (ичке) | E e, Ö ö, Ü ü, I i | K k, G g  |

## Alphabet (30 letters)

| #   | Lat | Cyr | IPA  |
| --- | --- | --- | ---- |
| 1   | A a | а   | /ɑ/  |
| 2   | B b | б   | /b/  |
| 3   | C c | дж  | /dʒ/ |
| 4   | Ç ç | ч   | /tʃ/ |
| 5   | D d | д   | /d/  |
| 6   | E e | е   | /e/  |
| 7   | F f | ф   | /f/  |
| 8   | G g | г   | /g/  |
| 9   | Ğ ğ | ғ   | /ɣ/  |
| 10  | H h | х   | /x/  |
| 11  | I i | и   | /i/  |
| 12  | J j | й   | /j/  |
| 13  | K k | к   | /k/  |
| 14  | Q q | қ   | /q/  |
| 15  | L l | л   | /l/  |
| 16  | M m | м   | /m/  |
| 17  | N n | н   | /n/  |
| 18  | Ñ ñ | ң   | /ŋ/  |
| 19  | O o | о   | /o/  |
| 20  | Ö ö | ө   | /ø/  |
| 21  | P p | п   | /p/  |
| 22  | R r | р   | /r/  |
| 23  | S s | с   | /s/  |
| 24  | Ş ş | ш   | /ʃ/  |
| 25  | T t | т   | /t/  |
| 26  | U u | у   | /u/  |
| 27  | Ü ü | ү   | /y/  |
| 28  | V v | в   | /v/  |
| 29  | Y y | ы   | /ɯ/  |
| 30  | Z z | з   | /z/  |

Extra letter for loanwords: **Ž ž** — soft Ж /ʒ/

## Rule 1: K/Q and G/Ğ Vowel Harmony

Cyrillic К and Г each split into two Latin letters depending on the vowel environment.

**Back vowels (a, o, u, y):** К → **Q**, Г → **Ğ**
**Front vowels (e, ö, ü, i):** К → **K**, Г → **G**

| Cyrillic | Latin (back) | Latin (front) |
| -------- | ------------ | ------------- |
| к        | Q q          | K k           |
| г        | Ğ ğ          | G g           |

### Examples

Back vowel words:

- карыя → qaryja
- кагаз → qağaz
- артка → artqa
- кымыз → qymyz
- кооз → qooz
- аргымак → arğymaq
- байкалган → bajqalğan

Front vowel words:

- көз → köz
- биргелешкен → birgeleşken
- үзгүлтүк → üzgültük

### Suffix harmony

| Suffix (Cyr) | Back | Front |
| ------------ | ---- | ----- |
| -ган/-ген    | -ğan | -gen  |
| -гон/-гөн    | -ğon | -gön  |
| -ка/-ке      | -qa  | -ke   |
| -ко/-кө      | -qo  | -kö   |
| -кы/-ки      | -qy  | -ki   |

## Rule 2: Я, Ё, Ю

Position determines the Latin representation.

### After vowels or at word start → ja, jo, ju

| Cyrillic | Latin  | Context                  |
| -------- | ------ | ------------------------ |
| Юра      | Jura   | word start               |
| оюн      | ojun   | after vowel              |
| аюу      | ajuu   | after vowel              |
| Аякс     | Ajaqs  | word start               |
| карыя    | qaryja | after vowel              |
| боёк     | bojoq  | after vowel              |
| Юлия     | Julija | word start + after vowel |

### After consonants → ä, ö, ü

| Cyrillic  | Latin      | Context                              |
| --------- | ---------- | ------------------------------------ |
| Кёльн     | Köln       | after consonant                      |
| жюри      | žüri       | after consonant                      |
| бюро      | büro       | after consonant                      |
| меню      | menü       | after consonant                      |
| лямбда    | lämbda     | after consonant                      |
| Семён     | Semön      | after consonant                      |
| Пётр      | Pötr       | after consonant                      |
| Брянск    | Bränsk     | after consonant                      |
| революция | revolüsija | mixed (ю after cons, ия after vowel) |
| Финляндия | Finländija | mixed                                |
| Женя      | Ženä       | after consonant                      |

## Rule 3: Ж Handling

Cyrillic Ж has two Latin representations depending on origin.

| Context                          | Latin        | Example                       |
| -------------------------------- | ------------ | ----------------------------- |
| Native Kyrgyz words              | **C c** /dʒ/ | (standard alphabet letter #3) |
| Loanwords, proper names (soft Ж) | **Ž ž** /ʒ/  | журнал → žurnal               |

### Ž examples

- журнал → žurnal
- Жарчысы журналы → Carçysy žurnaly
- Жириновский → Žyrinovskij
- донжуан → donžuan
- Брежнев → Brežnev
- жандармерия → žandarmerija
- Женя → Ženä
- жюри → žüri
- монтаж → montaž

Note: Native Kyrgyz Ж uses C (e.g. жайлоо → cailoo), while foreign/borrowed Ж uses Ž.

## Rule 4: Dialect Letters

Found in dialectal texts (southern/Osh dialect, Afghan Kyrgyz folklore).

| Letter | Sound          | Examples                                   |
| ------ | -------------- | ------------------------------------------ |
| Ä ä    | open front /æ/ | asäl (= bal), gäp (= kep), mäkä (= cügörü) |
| W w    | labial /w/     | dialectal only                             |

Sources: Murza Gaparov "Kyştakça", Sulajman Qajypov's Afghan Kyrgyz folklore works, J. Muqambaev's dialect dictionary.
