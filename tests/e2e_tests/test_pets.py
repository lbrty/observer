...
async def test_delete_pets_deletes_all_related_documents(authorized_client, app_context, consultant_user, env_settings):
    ...
    resp = await authorized_client.delete(f"/pets/{pet.id}")
    assert resp.status_code == status.HTTP_204_NO_CONTENT
    file_path = os.path.join(app_context.storage.root, env_settings.documents_path, str(pet.id))
    assert os.path.exists(file_path) is False


async def test_upload_document_for_pet_works(authorized_client, app_context, consultant_user, env_settings):
    project = await create_project(app_context, "test project", "test description")
    await create_permission(
        app_context,
        NewPermission(
    ...
    assert resp.status_code == status.HTTP_201_CREATED
    file_path = os.path.join(app_context.storage.root, env_settings.documents_path, str(pet.id))
    open(f"{file_path}/readme.md").read()
